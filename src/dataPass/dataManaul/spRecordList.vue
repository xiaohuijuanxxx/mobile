<template>
  <div style="overflow:hidden;">
    <template v-if="batchSwitch" >
       <flexbox v-if="!isAssistant" class="appfirstboxt" @click.native="onOrOff(recordStatus, dataList, 'dataList', 'recordStatus')">
        <flexbox-item :span="5">
          <div class="midFontSize" >手工数据备案审批</div>
        </flexbox-item>
        <flexbox-item :span="1">
          <x-icon v-show="!recordStatus" type="ios-arrow-down"></x-icon>
          <x-icon v-show="recordStatus" type="ios-arrow-up"></x-icon>
        </flexbox-item>
    ...
    <template v-else>
      ...
      >
        ...
        <flexbox-item :span="5">
          <div><span class="midFontSize">手工数据备案流程</span></div>
        </flexbox-item>
        <flexbox-item :span="3">
        ...
        <div class="parent">
          <el-checkbox-group v-model="checkedLists">
            <el-checkbox
              ...
              :label="item.dataRecordId"
              :key="item.dataRecordId"
            ...
            >
              <div
                ...
                @click="setCheckMyselfUser(item.dataRecordId)"
              ></div>
...
export default {
  ...
  methods: {
    ...
    turnOn(list, name, status){
      ...
      console.log(this[`${name}Mid`], this[status])
    },
    //收起
    ...
    changephotos(data) {
      ...
      data.length &&
        data.forEach((item) => {
          ...
          if (item.node == "5") {
            this.allCheckedOptions.push(item.dataRecordId);
          }
        });
      return data;
    },
    getData() {
      ...
      let url = URL.url.getRecordApproveList;
      ajaxGet(url, params)
        .then((res) => {
    ...
    toManualDetail(item) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          id: item.id,
          // type: type,
          status: item.dataRecordStatus ? item.dataRecordStatus : "",
          node: item.node,
          dataRecordUserId: item.dataRecordUserId, // 发起人账号
          userName: item.dataRecordUserName, // 发起人姓名
        },
